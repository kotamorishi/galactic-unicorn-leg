...
All CSS embedded. Minimal HTML to fit in ~192KB RAM.
Preset options are populated client-side via JS to save server memory.
"""
...
.btn{display:inline-block;padding:10px 20px;border:none;border-radius:10px;font-size:15px;font-weight:500;font-family:inherit;cursor:pointer;text-decoration:none}
...
.sc-days{font-size:12px;color:#8e8e93}.sc-sound{font-size:12px;color:#8e8e93;margin-top:2px}
...
def _page(title, body, extra_js=""):
    return '<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{t}</title><style>{c}</style></head><body><h1>{t}</h1>{b}<script>{j}{x}</script></body></html>'.format(
        t=title, b=body, c=_CSS, j=_JS_COMMON, x=extra_js)
...
def render_main_page(config, presets, status):
    ...
    clock = '<div class="clock"><span id="clock-time">{}</span><span class="clock-day" id="clock-day">{}</span></div>'.format(time_str, day_str)

    if status.get("active"):
        st = '{}<div class="status-on" id="st-label">Displaying</div><div class="status-msg" id="st-msg">{}</div><div class="status-sub" id="st-sub">Until {}</div>'.format(
            clock, _esc(status.get("message", "")), status.get("active_end", ""))
    elif status.get("next_start"):
        st = '{}<div class="status-off" id="st-label">Off</div><div class="status-msg" id="st-msg">{}</div><div class="status-sub" id="st-sub">Next: {} {}</div>'.format(
            clock, _esc(status.get("message", "")), status.get("next_day", ""), status.get("next_start", ""))
    else:
        st = '{}<div class="status-off" id="st-label">Off</div><div class="status-msg" id="st-msg">{}</div><div class="status-sub" id="st-sub">No schedules set</div>'.format(
            clock, _esc(status.get("message", "")))

    # Schedule items — compact, no preset options in HTML
    si = ""
    for s in schedules:
        days = ", ".join([d.capitalize() for d in s.get("days", [])]) or "Every day"
        snd = s.get("sound", {})
        si += '<div class="sc" data-id="{id}" data-start="{start}" data-end="{end}" data-en="{en}" data-snd-en="{se}" data-preset="{pi}" data-vol="{vol}" data-days="{dl}"><div class="sc-head"><label><input type="checkbox" class="sc-en" {chk}><span>{start} - {end}</span></label></div><div class="sc-days">{days}</div></div>'.format(
            id=s.get("id", 0),
            start=s.get("start_time", "00:00"),
            end=s.get("end_time", "23:59"),
            en="1" if s.get("enabled") else "0",
            chk="checked" if s.get("enabled") else "",
            se="1" if snd.get("enabled") else "0",
            pi=snd.get("preset_id", 1),
            vol=snd.get("volume", 50),
            dl=",".join(s.get("days", [])),
            days=days,
        )

    # Preset data as compact JS array: [[id,"name"],...]
    pa = "[" + ",".join(['[{},"{}"]'.format(p["id"], _esc(p["name"])) for p in presets]) + "]"

    body = '<div class="s"><h2>Now</h2>{st}</div>'.format(st=st)
    body += '<div class="s"><h2>Message</h2><input type="text" id="msg-text" value="{text}" maxlength="128" placeholder="Enter message"><div class="acts"><button class="btn bp" onclick="saveMsg()">Save</button></div><div id="msg-toast" class="toast"></div>'.format(
        text=_esc(msg.get("text", "")))
    body += '<details style="margin-top:10px"><summary>Options</summary><div class="row2" style="margin-top:8px"><div><label>Mode</label><select id="msg-mode"><option value="scroll" {ss}>Scroll</option><option value="fixed" {sf}>Fixed</option></select></div><div><label>Speed</label><select id="msg-speed"><option value="slow" {s1}>Slow</option><option value="medium" {s2}>Medium</option><option value="fast" {s3}>Fast</option></select></div></div><div class="row2"><div><label>Font</label><select id="msg-font"><option value="bitmap6" {f6}>Small</option><option value="bitmap8" {f8}>Normal</option></select></div><div><label>Color</label><input type="color" id="msg-color" value="{ch}"></div></div></details></div>'.format(
        ss='selected' if msg.get("display_mode") == "scroll" else "",
        sf='selected' if msg.get("display_mode") == "fixed" else "",
        s1='selected' if msg.get("scroll_speed") == "slow" else "",
        s2='selected' if msg.get("scroll_speed") == "medium" else "",
        s3='selected' if msg.get("scroll_speed") == "fast" else "",
        f6='selected' if msg.get("font") == "bitmap6" else "",
        f8='selected' if msg.get("font") == "bitmap8" else "",
        ch=color_hex)
    body += '<div class="s"><h2>Schedule</h2><div id="scheds">{si}</div><div class="acts"><button class="btn bs" onclick="addSc()">Add schedule</button><button class="btn bp" onclick="saveSc()">Save</button></div><div id="sc-toast" class="toast"></div></div>'.format(si=si)
    body += '<div class="s"><h2>Quick Settings</h2><label>Brightness</label><div class="rng"><input type="range" id="q-bright" min="0" max="100" value="{br}" oninput="setBright(this.value)"><span id="q-bright-v">{br}%</span></div><label>Volume</label><div class="rng"><input type="range" id="q-vol" min="0" max="100" value="50" oninput="setVol(this.value)"><span id="q-vol-v">50%</span></div></div>'.format(
        br=system.get("brightness", 50))
    body += '<div class="footer"><a href="/settings">Device Settings</a></div>'
...
function scEdit(el){{var d=el.dataset;var h='<details open><summary>Edit</summary><div class="row2" style="margin-top:8px"><div><label>Start</label><input type="time" class="sc-start" value="'+d.start+'"></div><div><label>End</label><input type="time" class="sc-end" value="'+d.end+'"></div></div><label>Days</label><div class="days-row">';var ds=(d.days||'').split(',');['mon','tue','wed','thu','fri','sat','sun'].forEach(function(x){{var ck=ds.indexOf(x)>=0?'checked':'';h+='<span class="dt"><input type="checkbox" data-day="'+x+'" '+ck+'>'+x.charAt(0).toUpperCase()+x.slice(1)+'</span>'}});h+='</div><label style="display:flex;align-items:center;gap:6px;color:#1c1c1e;margin-top:8px"><input type="checkbox" class="sc-snd-en" '+(d.sndEn=="1"?"checked":"")+'>Sound</label><div class="row2"><div><label>Preset</label><select class="sc-preset"></select></div><div><label>Volume</label><select class="sc-vol"><option value="25">25%</option><option value="50">50%</option><option value="75">75%</option><option value="100">100%</option></select></div></div><button class="btn bs bsm" onclick="playSnd(this)" style="margin-top:8px">Preview</button><div class="acts"><button class="btn bd bsm" onclick="delSc('+d.id+')">Remove</button></div></details>';if(!el.querySelector('details'))el.insertAdjacentHTML('beforeend',h);popts(el.querySelector('.sc-preset'),parseInt(d.preset));var vs=el.querySelector('.sc-vol');if(vs)vs.value=d.vol}}
...
function saveSc(){{var a=[];document.querySelectorAll('.sc').forEach(function(el){{var days=[];el.querySelectorAll('[data-day]').forEach(function(c){{if(c.checked)days.push(c.dataset.day)}});var st=el.querySelector('.sc-start'),en=el.querySelector('.sc-end');a.push({{id:parseInt(el.dataset.id),enabled:el.querySelector('.sc-en').checked,start_time:st?st.value:el.dataset.start,end_time:en?en.value:el.dataset.end,days:days.length?days:el.dataset.days.split(','),sound:{{enabled:el.querySelector('.sc-snd-en')?el.querySelector('.sc-snd-en').checked:false,preset_id:parseInt(el.querySelector('.sc-preset')?el.querySelector('.sc-preset').value:el.dataset.preset),volume:parseInt(el.querySelector('.sc-vol')?el.querySelector('.sc-vol').value:el.dataset.vol)}}}});}});api('POST','/api/schedules',a).then(function(){{toast('sc-toast','Saved')}}).catch(function(){{toast('sc-toast','Failed',1)}})}}
...
<div id="manual-row" style="display:none"><label>Network name</label><input type="text" id="wifi-ssid-manual" placeholder="SSID"></div>
<label>Password</label>
<input type="password" id="wifi-pass" placeholder="WiFi password">
<div class="acts"><button class="btn bp bsm" onclick="connectWifi()">Connect</button><button class="btn bs bsm" onclick="scanWifi()">Rescan</button></div>
...
<div class="acts"><button class="btn bs bsm" onclick="checkOta()">Check for updates</button><button class="btn bs bsm" onclick="reboot()">Reboot</button></div>
<div id="sys-toast" class="toast"></div></div>
<div class="footer"><a href="/">Back</a></div>""".format(
...
function reboot(){{if(confirm('Reboot device?'))api('POST','/api/system/reboot')}}"""
    ...
    return _page("Device Settings", body, js)
...
<div id="wifi-toast" class="toast"></div></div>""".format(options=options)

    js = """
function togManual(){{document.getElementById('manual-row').style.display=document.getElementById('wifi-ssid').value===''?'block':'none'}}
function connectWifi(){{var sel=document.getElementById('wifi-ssid');var m=document.getElementById('wifi-ssid-manual');var ssid=sel.value||(m?m.value:'');var pw=document.getElementById('wifi-pass').value;if(!ssid){{toast('wifi-toast','Enter a network name',1);return}}toast('wifi-toast','Saving...');api('POST','/api/wifi/connect',{{ssid:ssid,password:pw}}).then(function(r){{if(r.status=='saved')toast('wifi-toast','WiFi saved! Rebooting...');else toast('wifi-toast',r.error||'Failed',1)}}).catch(function(){{toast('wifi-toast','Failed',1)}})}}"""

    return _page("Welcome", body, js)
