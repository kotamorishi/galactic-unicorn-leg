...
function saveSc(){var a=[];document.querySelectorAll('.sc').forEach(function(el){var days=[];el.querySelectorAll('[data-day]').forEach(function(c){if(c.checked)days.push(c.dataset.day)});var st=el.querySelector('.sc-start'),en=el.querySelector('.sc-end'),mg=el.querySelector('.sc-msg'),cc=el.querySelector('.sc-color');var colVal=cc?cc.value:(el.dataset.color||'');a.push({id:parseInt(el.dataset.id),enabled:el.querySelector('.sc-en').checked,start_time:st?st.value:el.dataset.start,end_time:en?en.value:el.dataset.end,days:days.length?days:el.dataset.days.split(','),message:mg?mg.value:(el.dataset.msg||''),color:parseColor(colVal),sound:{enabled:el.querySelector('.sc-snd-en')?el.querySelector('.sc-snd-en').checked:false,preset_id:parseInt(el.querySelector('.sc-preset')?el.querySelector('.sc-preset').value:el.dataset.preset),volume:parseInt(el.querySelector('.sc-vol')?el.querySelector('.sc-vol').value:el.dataset.vol)}})});api('POST','/api/schedules',a).then(function(saved){S=[];SD=[];saved.forEach(function(s){var snd=s.sound||{},sc=s.color||{};var ch=(sc.r||sc.g||sc.b)?'#'+('0'+sc.r.toString(16)).slice(-2)+('0'+sc.g.toString(16)).slice(-2)+('0'+sc.b.toString(16)).slice(-2):'';S.push([s.id,s.start_time,s.end_time,s.enabled?1:0,snd.enabled?1:0,snd.preset_id||1,snd.volume||50,s.message||'',ch]);SD.push((s.days||[]).join(','))});renderSc();toast('sc-toast','Saved')}).catch(function(){toast('sc-toast','Failed',1)})}
