...
async def render_main_page(config, presets, status):
    ...
    # Compact schedule data as JS arrays: [id,start,end,enabled,sndEn,preset,vol,message]
    sd = []
    for s in schedules:
        snd = s.get("sound", {})
        sd.append("[{},{},{},{},{},{},{},{}]".format(
...
function saveMsg(){var c=document.getElementById('msg-color').value;api('POST','/api/message',{text:document.getElementById('msg-text').value,display_mode:document.getElementById('msg-mode').value,scroll_speed:document.getElementById('msg-speed').value,color:{r:parseInt(c.substr(1,2),16),g:parseInt(c.substr(3,2),16),b:parseInt(c.substr(5,2),16)},font:document.getElementById('msg-font').value}).then(function(){toast('msg-toast','Saved')}).catch(function(){toast('msg-toast','Failed',1)})}"""
    yield """
function scEdit(el){if(el.querySelector('details'))return;var d=el.dataset;var h='<details open><summary>Edit</summary><label style="margin-top:8px">Message</label><input type="text" class="sc-msg" value="'+esc(d.msg||'')+'" maxlength="128" placeholder="Message for this schedule"><div class="row2" style="margin-top:0"><div><label>Start</label><input type="time" class="sc-start" value="'+d.start+'"></div><div><label>End</label><input type="time" class="sc-end" value="'+d.end+'"></div></div><label>Days</label><div class="days-row">';var ds=(d.days||'').split(',');['mon','tue','wed','thu','fri','sat','sun'].forEach(function(x){h+='<span class="dt"><input type="checkbox" data-day="'+x+'" '+(ds.indexOf(x)>=0?'checked':'')+'>'+x.charAt(0).toUpperCase()+x.slice(1)+'</span>'});h+='</div><label style="display:flex;align-items:center;gap:6px;color:#1c1c1e;margin-top:8px"><input type="checkbox" class="sc-snd-en" '+(d.sndEn=="1"?"checked":"")+'>Sound</label><div class="row2"><div><label>Preset</label><select class="sc-preset"></select></div><div><label>Volume</label><select class="sc-vol"><option value="25">25%</option><option value="50">50%</option><option value="75">75%</option><option value="100">100%</option></select></div></div><button class="btn bs bsm" onclick="playSnd(this)" style="margin-top:8px">Preview</button><div class="acts"><button class="btn bd bsm" onclick="delSc('+d.id+')">Remove</button></div></details>';el.insertAdjacentHTML('beforeend',h);popts(el.querySelector('.sc-preset'),parseInt(d.preset));var vs=el.querySelector('.sc-vol');if(vs)vs.value=d.vol}
function renderSc(){var c=document.getElementById('scheds');c.innerHTML='';S.forEach(function(s,i){var id=s[0],st=s[1],en=s[2],on=s[3],se=s[4],pi=s[5],vo=s[6],msg=s[7]||'',ds=SD[i];var dd=ds?ds.split(',').map(function(x){return x.charAt(0).toUpperCase()+x.slice(1)}).join(', '):'Every day';var el=document.createElement('div');el.className='sc';el.dataset.id=id;el.dataset.start=st;el.dataset.end=en;el.dataset.en=on;el.dataset.sndEn=se;el.dataset.preset=pi;el.dataset.vol=vo;el.dataset.days=ds;el.dataset.msg=msg;var title=msg?esc(msg):'(no message)';el.innerHTML='<div class="sc-head"><label><input type="checkbox" class="sc-en" '+(on?'checked':'')+'>'+' <span style="font-weight:600">'+title+'</span></label></div><div class="sc-days" style="color:#8e8e93;font-size:12px">'+st+' - '+en+' / '+dd+'</div>';el.querySelector('.sc-head').addEventListener('click',function(e){if(e.target.type!='checkbox')scEdit(el)});c.appendChild(el);if(id>nid)nid=id})}
renderSc()"""
    yield """
function addSc(){nid++;var el=document.createElement('div');el.className='sc';el.dataset.id=nid;el.dataset.start='08:00';el.dataset.end='09:00';el.dataset.en='1';el.dataset.sndEn='0';el.dataset.preset='1';el.dataset.vol='50';el.dataset.days='mon,tue,wed,thu,fri,sat,sun';el.dataset.msg='';el.innerHTML='<div class="sc-head"><label><input type="checkbox" class="sc-en" checked><span>08:00 - 09:00</span></label></div><div class="sc-days">Every day</div>';document.getElementById('scheds').appendChild(el);scEdit(el)}
function delSc(id){var e=document.querySelector('[data-id="'+id+'"]');if(e)e.remove()}
function saveSc(){var a=[];document.querySelectorAll('.sc').forEach(function(el){var days=[];el.querySelectorAll('[data-day]').forEach(function(c){if(c.checked)days.push(c.dataset.day)});var st=el.querySelector('.sc-start'),en=el.querySelector('.sc-end'),mg=el.querySelector('.sc-msg');a.push({id:parseInt(el.dataset.id),enabled:el.querySelector('.sc-en').checked,start_time:st?st.value:el.dataset.start,end_time:en?en.value:el.dataset.end,days:days.length?days:el.dataset.days.split(','),message:mg?mg.value:(el.dataset.msg||''),sound:{enabled:el.querySelector('.sc-snd-en')?el.querySelector('.sc-snd-en').checked:false,preset_id:parseInt(el.querySelector('.sc-preset')?el.querySelector('.sc-preset').value:el.dataset.preset),volume:parseInt(el.querySelector('.sc-vol')?el.querySelector('.sc-vol').value:el.dataset.vol)}})});api('POST','/api/schedules',a).then(function(saved){S=[];SD=[];saved.forEach(function(s){var snd=s.sound||{};S.push([s.id,s.start_time,s.end_time,s.enabled?1:0,snd.enabled?1:0,snd.preset_id||1,snd.volume||50,s.message||'']);SD.push((s.days||[]).join(','))});renderSc();toast('sc-toast','Saved')}).catch(function(){toast('sc-toast','Failed',1)})}
