...
async def render_main_page(config, presets, status):
    ...
    # Message section
    yield '<div class="s"><h2>Message</h2><input type="text" id="msg-text" value="{}" maxlength="128" placeholder="Enter message"><div class="acts"><button class="btn bp" onclick="saveMsg()">Save</button><button class="btn bs" onclick="testSound()">Test Sound</button></div><div id="msg-toast" class="toast"></div>'.format(_esc(msg.get("text", "")))
    ...
    for s in schedules:
        ...
        color_hex = ""
        if sc_color and "r" in sc_color:
            color_hex = "#{:02x}{:02x}{:02x}".format(sc_color.get("r", 0), sc_color.get("g", 0), sc_color.get("b", 0))
        sd.append("[{},{},{},{},{},{},{},{},{}]".format(
            s.get("id", 0),
    ...
    # Quick settings
    boff = system.get("brightness_offset", 0)
    yield '<div class="s"><h2>Quick Settings</h2><label>Brightness Adjust (auto + offset)</label><div class="rng"><input type="range" id="q-bright" min="-50" max="50" value="{}" oninput="setBright(this.value)"><span id="q-bright-v">{}%</span></div><label>Volume</label><div class="rng"><input type="range" id="q-vol" min="0" max="100" value="50" oninput="setVol(this.value)"><span id="q-vol-v">50%</span></div></div>'.format(boff, boff)
...
function testSound(){api('POST','/api/sound/preview',{preset_id:1,volume:50})}
...
var _bt=0,_vt=0;
function setBright(v){var n=parseInt(v);document.getElementById('q-bright-v').textContent=(n>=0?'+':'')+n+'%';clearTimeout(_bt);_bt=setTimeout(function(){api('POST','/api/system/brightness',{brightness_offset:n})},1000)}
function setVol(v){document.getElementById('q-vol-v').textContent=v+'%';clearTimeout(_vt);_vt=setTimeout(function(){api('POST','/api/system/volume',{volume:parseInt(v)})},1000)}"""
    yield """
function fetchStatus(){api('GET','/api/status').then(function(s){if(s.time)document.getElementById('clock-time').textContent=s.time;if(s.day)document.getElementById('clock-day').textContent=s.day;var l=document.getElementById('st-label'),u=document.getElementById('st-sub');if(s.active){l.textContent='Displaying';l.className='status-on';u.textContent='Until '+s.active_end}else if(s.next_start){l.textContent='Off';l.className='status-off';u.textContent='Next: '+s.next_day+' '+s.next_start}else{l.textContent='Off';l.className='status-off';u.textContent='No schedules set'}document.getElementById('st-msg').textContent=s.message||'';if('brightness_offset' in s){var sl=document.getElementById('q-bright'),lb=document.getElementById('q-bright-v');if(sl){sl.value=s.brightness_offset;lb.textContent=(s.brightness_offset>=0?'+':'')+s.brightness_offset+'%'}}}).catch(function(){})}
fetchStatus();
